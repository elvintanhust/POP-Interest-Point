...
class SuperPointFrontend(object):
    ...
    def run(self, img, tensor_rgb):
        ...
        # detect point by our network, and replace original points
        with torch.no_grad():
            tensor_rgb = tensor_rgb.to(self.device)
            score_output, coarse_desc = self.net(tensor_rgb)
            # 转换为概率值
            point_resp = torch.sigmoid(score_output).detach()
        heatmap = point_resp.cpu().numpy().squeeze()
        xs, ys = np.where(heatmap >= 0.5)
...
class VideoStreamer(object):
    ...
    def read_image(self, impath, img_size):
        ...
        grayim = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY)
        if grayim is None:
        ...
        tensor_rgb = img_rgb[np.newaxis, :]
        tensor_rgb = tensor_rgb.transpose([0, 3, 1, 2])
        tensor_rgb = (torch.from_numpy(tensor_rgb).float() - 128) / 129
        return grayim, tensor_rgb
    ...
    def next_frame(self):
        ...
        if self.camera:
            ret, input_image_rgb = self.cap.read()
            if ret is False:
                print('VideoStreamer: Cannot get image from camera (maybe bad --camid?)')
                return (None, False, None)
            if self.video_file:
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.listing[self.i])
            input_image_rgb = cv2.resize(input_image_rgb, (self.sizer[1], self.sizer[0]),
                                         interpolation=cv2.INTER_AREA)
            input_image = cv2.cvtColor(input_image_rgb, cv2.COLOR_RGB2GRAY)
            input_image = input_image.astype('float') / 255.0

            tensor_rgb = input_image_rgb[np.newaxis, :]
            tensor_rgb = tensor_rgb.transpose([0, 3, 1, 2])
            tensor_rgb = (torch.from_numpy(tensor_rgb).float() - 128) / 129
        else:
            image_file = self.listing[self.i]
            input_image, tensor_rgb = self.read_image(image_file, self.sizer)
        # Increment internal counter.
        self.i = self.i + 1
        input_image = input_image.astype('float32')
        return (input_image, True, tensor_rgb)
...
if __name__ == '__main__':
    ...
    # 写入视频
    video_reulst_name = os.path.join('video_result', '3.mp4')
    fps = 25
    size = (640, 480)
    video_writer = cv2.VideoWriter(video_reulst_name, cv2.VideoWriter_fourcc(*'DIVX'), fps, size)
    frame_now = 0
    while True:
        ...
        # Get a new image.
        img, status, img_rgb = vs.next_frame()
        if status is False:
            break

        # Get points and descriptors.
        start1 = time.time()
        pts, desc, heatmap = fe.run(img, img_rgb)
        end1 = time.time()
